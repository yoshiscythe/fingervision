...
base_dir = "/home/suzuki/ros_ws/ay_tools/fingervision/suzuki/rubbing_hand/data/1225/"
...
def create_boxplot_graph2(dfs, x_label, x_label_display, y_label, y_label_display, ax, colors, labels):
    ...
    sns.boxplot(x=x_label, y=y_label, hue="series", data=cdf, ax=ax, width=0.5)
    # ax.set_xticklabels(x)
    ax.set_xlabel(x_label_display, fontsize=fontsize)
    ax.set_ylabel(y_label_display, fontsize=fontsize)
...
# rmse of angular velocity
def create_barplot_graph3(dfs, x_label, x_label_display, y_label, y_label_display, ax, colors, labels):
    df_list = []
    for (df, label) in zip(dfs, labels):
        # print(df, label)
        data_dict = {}
        for index, row in df.iterrows():
            row = row.to_dict()
            x_data = row[x_label]
            rmse_sum = row[y_label+"_sum"]
            rmse_len = row[y_label+"_len"]
            data = np.array([rmse_sum, rmse_len])
            # print(type(y_data), y_data, data)
            if not x_data in data_dict:
                data_dict[x_data] = data
