...
# autoconnector.shからコピペ

PROGNAME=$(basename "$0")
DIRNAME=$(dirname "$0")

FILE=$DIRNAME/bt_list
...
function main() {
  ...
  if ! $is_mapping_file; then # attempts to connect all devices
    bluetoothctl -- list | while read line; do
      adapter=$(echo "$line" | sed -r 's/^.*(([0-9A-F]{2}:){5}[0-9A-F]{2}).*$/\1/')

      paired_devices | while read device; do
  ...
  else # attempts to disconnect specific devices written in mapping list
    while read line; do
      ...
      if ! [[ $line =~ ^# ]] && ! [[ $line =~ ^([[:blank:]]+.*)*$ ]]; then
        ...
        # disconnect the device if not connected
        if [[ $(is_connected) = "yes" ]]; then
          echo Attempting to disconnect from "$name"...
          {
            printf "select $adapter\n\n"
            printf "disconnect $device\n\n"
          } | bluetoothctl &>/dev/null
          echo "Cheking if it's disconnected..."
          sleep 1
          if [[ $(is_connected) = "no" ]]; then
            echo Succeeded to disconnect from "$name"!
          else
            echo Failed to disconnect from "$name"
          fi
        else
          echo "$name" is already disconnected!
        fi
      fi
    done <"$FILE"
  fi
...
# checks the device's name
# returns the device's name
function device_name() {
  {
    printf "select $adapter\n\n"
    printf "info $device\n\n"
  } | bluetoothctl | grep "Name: " | sed -e 's/Name: //' | sed -e 's/^[[:blank:]]*//'
}
