...
class TDynamixel1(object):
  ...
  #Two's complement
  #https://qiita.com/shohein/items/d73ea8483732f09ba37f
  def convert_complement_on_two_into_decimal(self, bits):
    return -int(bits[0]) << len(bits) | int(bits, 2)

  # Macro for Control Table Value
  def DXL_MAKEWORD(self, a, b):
      return (a & 0xFF) | ((b & 0xFF) << 8)


  def DXL_MAKEDWORD(self, a, b):
      return (a & 0xFFFF) | (b & 0xFFFF) << 16


  def DXL_LOWORD(self, l):
      return l & 0xFFFF


  def DXL_HIWORD(self, l):
      return (l >> 16) & 0xFFFF


  def DXL_LOBYTE(self, w):
      return w & 0xFF


  def DXL_HIBYTE(self, w):
      return (w >> 8) & 0xFF

  def SetupPosSyncWrite(self):
    addr,size= self.ADDR["GOAL_POSITION"]
    self.groupsyncwrite = dynamixel.group_sync_write.GroupSyncWrite(self.port_handler(), self.packet_handler, addr, size)
  def SetupPosSyncRead(self):
    addr,size= self.ADDR["PRESENT_POSITION"]
    self.groupsyncread_pos = dynamixel.group_sync_read.GroupSyncRead(self.port_handler(), self.packet_handler, addr, size)
  def SetupVelSyncRead(self):
    addr,size= self.ADDR["PRESENT_VELOCITY"]
    self.groupsyncread_vel = dynamixel.group_sync_read.GroupSyncRead(self.port_handler(), self.packet_handler, addr, size)
  def SetupPwmSyncRead(self):
    addr,size= self.ADDR["PRESENT_PWM"]
    self.groupsyncread_pwm = dynamixel.group_sync_read.GroupSyncRead(self.port_handler(), self.packet_handler, addr, size)
  def SetupSyncRead(self):
    addr, size = 124, 12
    self.groupsyncread = dynamixel.group_sync_read.GroupSyncRead(self.port_handler(), self.packet_handler, addr, size)
    for id in range(4):
      id += 1
      self.groupsyncread.addParam(id)
  def SetupBulkRead(self):
    self.groupbulkread = dynamixel.GroupBulkRead(self.port_handler(), self.packet_handler)

  def SetPosition4PosSync(self, target):
    for id in range(len(target)):
      bytes_data = [self.DXL_LOBYTE(self.DXL_LOWORD(target[id])), self.DXL_HIBYTE(self.DXL_LOWORD(target[id])), self.DXL_LOBYTE(self.DXL_HIWORD(target[id])), self.DXL_HIBYTE(self.DXL_HIWORD(target[id]))]
      result = self.groupsyncwrite.addParam(id + 1, bytes_data)
      # print(target[id],bytes_data, len(bytes_data), result)
    self.groupsyncwrite.txPacket()
    self.groupsyncwrite.clearParam()

  def ReadPosSync(self):
    addr,size= self.ADDR["PRESENT_POSITION"]
    ...
    for id in range(4):
      id += 1
      dxl_getdata_result = self.groupsyncread_pos.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupSyncRead Pos getdata failed" % id)
        quit()
      get_pos.append(self.groupsyncread_pos.getData(id, addr, size))
    return get_pos

  def ReadVelSync(self):
    addr,size= self.ADDR["PRESENT_VELOCITY"]
    dxl_comm_result = self.groupsyncread_vel.txRxPacket()
    if dxl_comm_result != dynamixel.COMM_SUCCESS:
      print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
    get_vel =[]
    for id in range(4):
      id += 1
      dxl_getdata_result = self.groupsyncread_vel.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupSyncRead Vel getdata failed" % id)
        quit()
      get_vel.append(self.groupsyncread_vel.getData(id, addr, size))
    return get_vel

  def ReadPwmSync(self):
    addr,size= self.ADDR["PRESENT_PWM"]
    dxl_comm_result = self.groupsyncread_pwm.txRxPacket()
    if dxl_comm_result != dynamixel.COMM_SUCCESS:
      print("%s" % self.packet_handler.getTxRxResult(dxl_comm_result))
    get_pwm =[]
    for id in range(4):
      id += 1
      dxl_getdata_result = self.groupsyncread_pwm.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupSyncRead Pwm getdata failed" % id)
        quit()
      get_pwm.append(self.groupsyncread_pwm.getData(id, addr, size))
    return get_pwm

  def ReadAllSync(self):
    ...
    #     print("[ID:%03d] groupSyncRead Pos getdata failed" % id)
    #     quit()
    #   get_pos.append(self.groupsyncread.getData(id, addr, size))
    ...
    #     print("[ID:%03d] groupSyncRead Vel getdata failed" % id)
    #     quit()
    #   get_vel.append(self.groupsyncread.getData(id, addr, size))
    ...
    #   dxl_getdata_result = self.groupsyncread.isAvailable(id, addr, size)
    #   if dxl_getdata_result != True:
    #     print("[ID:%03d] groupSyncRead All getdata failed" % id)
    #     quit()
    #   get_pos.append(self.groupsyncread.getData(id, addr, size))

  def ReadXSync(self, address):
  ...
  def ReadPosBulk(self):
    addr,size= self.ADDR["PRESENT_POSITION"]
    for id in range(4):
      id += 1
      dxl_addparam_result = self.groupbulkread.addParam(id, addr, size)
    ...
    for id in range(4):
      id += 1
      addr,size= self.ADDR["PRESENT_POSITION"]
      dxl_getdata_result = self.groupbulkread.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupBulkRead Pos getdata failed" % id)
        quit()
      get_pos.append(self.fix_value(self.groupbulkread.getData(id, addr, size), size))
    self.groupbulkread.clearParam()
    return get_pos

  def ReadVelBulk(self):
    addr,size= self.ADDR["PRESENT_VELOCITY"]
    for id in range(4):
      id += 1
      dxl_addparam_result = self.groupbulkread.addParam(id, addr, size)
    ...
    for id in range(4):
      ...
      dxl_getdata_result = self.groupbulkread.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupBulkRead Vel getdata failed" % id)
        quit()
      get_vel.append(self.fix_value(self.groupbulkread.getData(id, addr, size), size))
    self.groupbulkread.clearParam()
    return get_vel

  def ReadPwmBulk(self):
    addr,size= self.ADDR["PRESENT_PWM"]
    for id in range(4):
      id += 1
      dxl_addparam_result = self.groupbulkread.addParam(id, addr, size)
    ...
    for id in range(4):
      ...
      dxl_getdata_result = self.groupbulkread.isAvailable(id, addr, size)
      if dxl_getdata_result != True:
        print("[ID:%03d] groupBulkRead PWM getdata failed" % id)
        quit()
      get_pwm.append(self.fix_value(self.groupbulkread.getData(id, addr, size), size))
    self.groupbulkread.clearParam()
    return get_pwm
  ...
  def Read(self, address):
    ...
    if size == 4:
      value = self.convert_complement_on_two_into_decimal(format(value, '032b'))

    return value

  def fix_value(self, value, size):
    if size==2:
      #value= value & 255
      #if value>127:  value= -(256-value)
      #value= value & 1023
  ...
  def SetOpMode(self, mode):
    ...
    self.Write('VEL_LIMIT', self.VelocityLimit)
  ...
  def SetVelocity(self, target):
    target = int(target)
    #FIXME: If OpMode allows multi turn, target could vary.
    if target < -self.VelocityLimit:  target = -self.VelocityLimit
    elif target > self.VelocityLimit:  target = self.VelocityLimit

    # Write goal velocity
    self.Write('GOAL_VELOCITY', target)
    #if not self.CheckTxRxResult():  return
    self.CheckTxRxResult()
    #print 'debug:TDynamixel1:MoveTo:',target
  ...
  def SetPosGain(self, kp, ki, kd):
    self.Write("POS_P_GAIN", kp)
    self.Write("POS_I_GAIN", ki)
    self.Write("POS_D_GAIN", kd)
    self.CheckTxRxResult()
...
if __name__=='__main__':
  print( type(dynamixel))
  for x in dir(dynamixel):
    print( x, ':', type(eval("dynamixel."+x)))
  # import os
